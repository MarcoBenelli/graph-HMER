...
for mode in modes:
    geda = GraphEditDistanceAlgorithm(mode)
    ...
    for query_inkml in os.scandir(query_inkml_path):
        ...
        for inkml in os.scandir(inkml_path):
            ...
            symbols_list = [symbol.text for symbol in
                            tree.findall('InkML:traceGroup/InkML:traceGroup/InkML:annotation', ns)]
            ...
            print(colored('Checked expressions = ' + str(processed_expressions) + ' / ' + str(num_expressions),
                          'magenta'))
    ...
    print(mode, 'recall =', recall)
    print(mode, 'precision =', precision)
    recall.reverse()
    precision.reverse()
    interpolated_recall, interpolated_precision = interpolate_precision(recall, precision)
    ax.plot(interpolated_recall, interpolated_precision, label=mode)

ax.legend()
fig.savefig(plots_path + 'plot.png')
print()
